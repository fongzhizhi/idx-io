...
export class Polyline extends Geometry2D {
    ...
    for (const primitive of this.primitives) {
      if (primitive instanceof Line) {
        if (vertices.length === 0) {
          vertices.push(primitive.start);
        }
        vertices.push(primitive.end);
      } else if (primitive instanceof Arc) {
        // 对圆弧进行离散化
        const arc = primitive;
        const segments = Math.ceil(Math.abs(arc.sweepAngle) / (Math.PI / 12)); // 每15度一个点
        for (let i = 0; i <= segments; i++) {
          const t = i / segments;
          const angle = arc.startAngle + arc.sweepAngle * t;
          vertices.push(arc.getPointAtAngle(angle));
        }
      }
    }
  ...
  transform(matrix: Matrix3): Polyline {
    const transformedPrimitives = this.primitives.map(prim => 
      prim.transform(matrix) as Geometry2D
    );
  ...
  reverse(): Polyline {
    ...
    for (let i = this.primitives.length - 1; i >= 0; i--) {
      ...
      if (primitive instanceof Line) {
        reversedPrimitives.push(new Line(primitive.end, primitive.start));
      } else if (primitive instanceof Arc) {
        reversedPrimitives.push(new Arc(
          primitive.radiusX,
          primitive.radiusY,
          primitive.center,
          primitive.endAngle,
  ...
  getLength(): number {
    return this.primitives.reduce((total, primitive) => {
      if (primitive instanceof Line) {
        ...
        // 圆弧长度近似计算
        const arc = primitive;
        return total + Math.sqrt(arc.radiusX * arc.radiusX + arc.radiusY * arc.radiusY) * 
